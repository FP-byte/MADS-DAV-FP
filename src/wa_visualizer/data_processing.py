...
class Preprocess(DataObject):
    ...
    def prepocess_week2(self, startdate, enddate):          
    ...
    def preprocess_week3(self):
        ...
        df['contains_eten'] = contains_keywords(df['message'], eten_keywords)
        df['contains_plans'] = contains_keywords(df['message'], plans_keywords)

        # Filter DataFrame to remove rows that contain 'eten'
        df2 = df[~df['contains_eten']]

        # Check for places in the filtered DataFrame
        df['contains_place'] = contains_keywords(df2['message'], place_keywords)

        # Optionally, if you need another filtered DataFrame
        df3 = df2[~df2['contains_eten']] 

        df['contains_people'] = contains_keywords(df3['message'], people_keywords)
